	...
	public function approve(int $job_id) {
		...
		if (! $processed_attachment_id) {
			if (empty($job['processed_url']) || ! wp_http_validate_url((string) $job['processed_url'])) {
				$error = new WP_Error('catalogue_image_studio_missing_processed_image', __('Processed image missing. Reprocess before approving.', 'catalogue-image-studio'));
			...
			if (is_wp_error($processed_attachment_id)) {
				$error = new WP_Error('catalogue_image_studio_missing_processed_image', __('Processed image could not be found. Reprocess this image.', 'catalogue-image-studio'));
				$this->mark_approval_error((int) $job_id, $error);
				return $error;
			}
		}
