...
class Catalogue_Image_Studio_SEO_Metadata_Generator {
	...
	public function generate(int $product_id, int $attachment_id, string $slot, string $original_filename, string $processed_filename, array $settings): array {
		...
		if ('' === $title) {
			$title = __('Product', 'optivra-image-studio');
		}
		...
		$product_type    = '' !== $category_phrase ? $category_phrase : __('product accessory', 'optivra-image-studio');
		...
		$alt = $this->trim_sentence(
			sprintf(
				...
				__('%1$s, shown as %2$s.', 'optivra-image-studio'),
				$title,
		...
		$caption = $this->trim_sentence(
			sprintf(
				/* translators: 1: product name, 2: product/category phrase */
				__('%1$s for %2$s.', 'optivra-image-studio'),
				$title,
		...
		$description_bits = array_filter([
			sprintf(
				/* translators: 1: product name, 2: product/category phrase */
				__('Optimised product image for %1$s, a %2$s', 'optivra-image-studio'),
				$title,
				strtolower($product_type)
			),
			'' !== $store ? sprintf(__('available from %s', 'optivra-image-studio'), $store) : '',
		]);
		$description = rtrim(implode(' ', $description_bits), '.') . '.';

		if ('' !== $sku) {
			$description .= ' ' . sprintf(__('SKU: %s.', 'optivra-image-studio'), $sku);
		}
