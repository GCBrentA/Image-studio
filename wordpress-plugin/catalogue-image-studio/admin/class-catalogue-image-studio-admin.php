...
class Catalogue_Image_Studio_Admin {
	...
	public function register_menu(): void {
		...
		$this->page_hook = add_submenu_page(
			'woocommerce',
			__('Optivra Image Studio', 'optivra-image-studio'),
			__('Optivra Image Studio', 'optivra-image-studio'),
			'manage_woocommerce',
			'optivra',
			[$this, 'render_page']
	...
	public function handle_settings_post(): void {
		...
		if (isset($_POST['disconnect_store'])) {
			...
			$this->queue_notice(__('Store disconnected.', 'optivra-image-studio'), 'success');
		...
		if (! $saved && get_option($this->plugin->get_option_name()) !== $settings) {
			...
			$this->queue_notice(__('Settings could not be saved. Please try again.', 'optivra-image-studio'), 'error');
			$this->redirect_after_settings_post();
			return;
		}
		...
		if (isset($_POST['clear_local_cache'])) {
			...
			$this->queue_notice(__('Local image job cache cleared.', 'optivra-image-studio'), 'success');
			$this->redirect_after_settings_post();
			return;
		}
		...
		if (isset($_POST['reset_local_data'])) {
			$this->plugin->jobs()->delete_all();
			$this->queue_notice(__('Plugin local data reset.', 'optivra-image-studio'), 'success');
			$this->redirect_after_settings_post();
			return;
		}
		...
		if (isset($_POST['test_connection']) || isset($_POST['connect_store'])) {
			...
			if (is_wp_error($usage)) {
				...
				$this->queue_notice(__('Store connected to Optivra.', 'optivra-image-studio'), 'success');
			}
		} else {
			$this->queue_notice(__('Settings saved.', 'optivra-image-studio'), 'success');
		}
	...
	public function sanitize_settings_payload($input): array {
		...
		$placeholder_token = __('Token saved - leave blank to keep it', 'optivra-image-studio');
	...
	public function handle_workflow_post(): void {
		...
		if (is_wp_error($usage)) {
			$this->add_error(__('Connect your Optivra account before scanning or processing images.', 'optivra-image-studio'));
			return;
		}
		...
		if ('scan' === $action) {
			...
			if ((int) $result['slots_found'] <= 0) {
				$this->add_error(__('No images found. Check product status/category filters or run with debug mode enabled.', 'optivra-image-studio'));
			} else {
				$this->add_success(
					sprintf(
						/* translators: 1: slots found, 2: jobs created/refreshed */
						__('Scan complete. Found %1$d images and refreshed %2$d jobs.', 'optivra-image-studio'),
		...
		if ('queue_visible' === $action || 'queue_category' === $action) {
			...
			$this->add_success(sprintf(
				...
				__('%d image(s) queued.', 'optivra-image-studio'),
				count((array) $result['job_ids'])
			));
			return;
		}
		...
		if ('queue_selected_slots' === $action) {
			...
			$this->add_success(sprintf(
				/* translators: %d: jobs queued */
				__('%d selected image(s) queued.', 'optivra-image-studio'),
				count($job_ids)
			));
			return;
		}
		...
		if ('process_next_batch' === $action) {
			...
			if (! empty($settings['pause_low_credits']) && $this->is_low_credit_state($usage)) {
				$this->add_error(__('Processing is paused because credits are running low. Buy credits or disable the pause setting to continue.', 'optivra-image-studio'));
				return;
			}
		...
		if (empty($job_ids)) {
			$this->add_error('approve' === $action ? __('Select at least one image to approve.', 'optivra-image-studio') : __('Select at least one image.', 'optivra-image-studio'));
			return;
		}
		...
		if ('process' === $action) {
			...
			if ($credits_remaining < count($job_ids)) {
				...
				$this->add_error(__('Insufficient credits. Processing available images only; buy credits or upgrade plan to process the rest.', 'optivra-image-studio'));
			}

			if (empty($job_ids)) {
				$this->add_error(__('No credits available. Buy credits or upgrade plan to continue.', 'optivra-image-studio'));
				return;
			}
		}
		...
		if ('cancel' === $action) {
			...
			$this->add_success(sprintf(
				...
				__('%d image(s) removed from queue.', 'optivra-image-studio'),
				$updated
			));
			return;
		}
			...
			if ('approve' === $action) {
				$this->add_success(
					sprintf(
						...
						_n('%d image approved and applied.', '%d images approved and applied.', $success, 'optivra-image-studio'),
						$success
					)
				);
				$success = 0;
		...
		if ($success > 0) {
			$this->add_success(
				sprintf(
					...
					__('%1$s complete for %2$d image(s).', 'optivra-image-studio'),
		...
		if ($failed > 0) {
			$this->add_error(
				sprintf(
					/* translators: %d: count */
					__('%d image action(s) failed.', 'optivra-image-studio'),
					$failed
				)
			);
		}
	...
	private function run_job_action(string $action, int $job_id) {
		switch ($action) {
			...
			default:
				return new WP_Error('catalogue_image_studio_unknown_action', __('Unknown action.', 'optivra-image-studio'));
		}
	}
	...
	private function regenerate_job_seo(int $job_id) {
		...
		if (! $job) {
			return new WP_Error('catalogue_image_studio_missing_job', __('Job not found.', 'optivra-image-studio'));
		}
	...
	public function render_page(): void {
		if (! current_user_can('manage_woocommerce')) {
			wp_die(esc_html__('You do not have permission to access this page.', 'optivra-image-studio'));
		}
		...
		<div class="wrap catalogue-image-studio-admin">
			<h1><?php echo esc_html__('Optivra Image Studio', 'optivra-image-studio'); ?></h1>
			<p class="catalogue-image-studio-page-intro"><?php echo esc_html__('Configure how Optivra optimises and writes product images back to your store.', 'optivra-image-studio'); ?></p>
	...
	private function render_dashboard_tab(array $settings, array $usage, array $jobs): void {
		...
		<div class="catalogue-image-studio-panel">
			<div class="catalogue-image-studio-dashboard-head">
				<div>
					<h2><?php echo esc_html__('Welcome to Optivra Image Studio', 'optivra-image-studio'); ?></h2>
					<?php $this->render_usage($usage); ?>
				</div>
				<div class="catalogue-image-studio-cta-buttons">
					<a class="button button-primary" href="<?php echo esc_url(add_query_arg(['page' => 'optivra', 'cis_tab' => 'scan'], admin_url('admin.php'))); ?>"><?php echo esc_html__('Scan Catalogue', 'optivra-image-studio'); ?></a>
					<a class="button" href="<?php echo esc_url(add_query_arg(['page' => 'optivra', 'cis_tab' => 'review'], admin_url('admin.php'))); ?>"><?php echo esc_html__('Review Images', 'optivra-image-studio'); ?></a>
					<a class="button" href="<?php echo esc_url($this->get_buy_credits_url($usage, $settings)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Buy Credits', 'optivra-image-studio'); ?></a>
					<a class="button" href="<?php echo esc_url($this->get_account_url($usage, $settings)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Manage Account', 'optivra-image-studio'); ?></a>
				</div>
			</div>
			<div class="catalogue-image-studio-metrics">
				<?php
				$this->render_metric(__('Images scanned', 'optivra-image-studio'), $images_scanned);
				$this->render_metric(__('Queued', 'optivra-image-studio'), (int) ($counts['queued'] ?? 0));
				$this->render_metric(__('Processing', 'optivra-image-studio'), (int) ($counts['processing'] ?? 0));
				$this->render_metric(__('Completed', 'optivra-image-studio'), (int) ($counts['completed'] ?? 0));
				$this->render_metric(__('Failed', 'optivra-image-studio'), (int) ($counts['failed'] ?? 0));
				$this->render_metric(__('Awaiting approval', 'optivra-image-studio'), $awaiting_approval);
				?>
			</div>
		</div>
		<?php
	...
	private function render_scan_tab(): void {
		...
		?>
		<div class="catalogue-image-studio-panel">
			<h2><?php echo esc_html__('Scan Catalogue', 'optivra-image-studio'); ?></h2>
			...
			<form method="post" action="">
				...
				<div class="catalogue-image-studio-toolbar">
					<strong><?php echo esc_html(sprintf(__('%d image(s) visible', 'optivra-image-studio'), count($slots))); ?></strong>
					<div class="catalogue-image-studio-cta-buttons">
						<button type="submit" name="catalogue_image_studio_action" value="scan" class="button"><?php echo esc_html__('Scan all product images', 'optivra-image-studio'); ?></button>
						<button type="submit" name="catalogue_image_studio_action" value="queue_selected_slots" class="button button-primary"><?php echo esc_html__('Queue selected', 'optivra-image-studio'); ?></button>
						<button type="submit" name="catalogue_image_studio_action" value="queue_visible" class="button"><?php echo esc_html__('Queue all visible', 'optivra-image-studio'); ?></button>
						<button type="submit" name="catalogue_image_studio_action" value="queue_category" class="button"><?php echo esc_html__('Queue category', 'optivra-image-studio'); ?></button>
					</div>
				</div>
				<table class="widefat fixed striped catalogue-image-studio-jobs">
					<thead><tr><td class="check-column"><input type="checkbox" class="catalogue-image-studio-check-all" onclick="document.querySelectorAll('.catalogue-image-studio-job-check').forEach((box) => box.checked = this.checked); catalogueImageStudioUpdateSelectedCount();" /></td><th><?php echo esc_html__('Image', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('Product', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('Slot', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('Status', 'optivra-image-studio'); ?></th></tr></thead>
					<tbody>
						<?php if (empty($slots)) : ?>
							<tr><td colspan="5"><?php echo esc_html__('No images found. Check product status/category filters or run with debug mode enabled.', 'optivra-image-studio'); ?></td></tr>
						<?php else : ?>
	...
	private function render_scan_filters(array $filters): void {
		...
		<form method="get" action="" class="catalogue-image-studio-filters">
			...
			<label><span><?php echo esc_html__('Category', 'optivra-image-studio'); ?></span><select name="filter_category"><option value="0"><?php echo esc_html__('All categories', 'optivra-image-studio'); ?></option><?php if (! is_wp_error($categories)) : foreach ($categories as $category) : ?><option value="<?php echo esc_attr((string) $category->term_id); ?>" <?php selected((int) ($filters['category'] ?? 0), (int) $category->term_id); ?>><?php echo esc_html($category->name); ?></option><?php endforeach; endif; ?></select></label>
			<label><span><?php echo esc_html__('Product status', 'optivra-image-studio'); ?></span><select name="filter_product_status"><option value="publish" <?php selected('publish', (string) ($filters['status'] ?? 'publish')); ?>><?php echo esc_html__('Published', 'optivra-image-studio'); ?></option><option value="draft" <?php selected('draft', (string) ($filters['status'] ?? 'publish')); ?>><?php echo esc_html__('Draft', 'optivra-image-studio'); ?></option><option value="private" <?php selected('private', (string) ($filters['status'] ?? 'publish')); ?>><?php echo esc_html__('Private', 'optivra-image-studio'); ?></option><option value="all" <?php selected('all', (string) ($filters['status'] ?? 'publish')); ?>><?php echo esc_html__('All', 'optivra-image-studio'); ?></option></select></label>
			<label><span><?php echo esc_html__('Product type', 'optivra-image-studio'); ?></span><select name="filter_product_type"><option value=""><?php echo esc_html__('All types', 'optivra-image-studio'); ?></option><option value="simple" <?php selected('simple', (string) ($filters['product_type'] ?? '')); ?>><?php echo esc_html__('Simple', 'optivra-image-studio'); ?></option><option value="variable" <?php selected('variable', (string) ($filters['product_type'] ?? '')); ?>><?php echo esc_html__('Variable', 'optivra-image-studio'); ?></option></select></label>
			<label><span><?php echo esc_html__('Processing state', 'optivra-image-studio'); ?></span><select name="filter_processing_state"><option value=""><?php echo esc_html__('All', 'optivra-image-studio'); ?></option><option value="unprocessed" <?php selected('unprocessed', isset($_GET['filter_processing_state']) ? sanitize_key(wp_unslash($_GET['filter_processing_state'])) : ''); ?>><?php echo esc_html__('Unprocessed', 'optivra-image-studio'); ?></option><option value="processed" <?php selected('processed', isset($_GET['filter_processing_state']) ? sanitize_key(wp_unslash($_GET['filter_processing_state'])) : ''); ?>><?php echo esc_html__('Already processed', 'optivra-image-studio'); ?></option></select></label>
			<label><span><?php echo esc_html__('Stock', 'optivra-image-studio'); ?></span><select name="filter_stock_status"><option value=""><?php echo esc_html__('Any stock status', 'optivra-image-studio'); ?></option><option value="instock" <?php selected('instock', (string) ($filters['stock_status'] ?? '')); ?>><?php echo esc_html__('In stock', 'optivra-image-studio'); ?></option><option value="outofstock" <?php selected('outofstock', (string) ($filters['stock_status'] ?? '')); ?>><?php echo esc_html__('Out of stock', 'optivra-image-studio'); ?></option></select></label>
			<label class="catalogue-image-studio-filter-check"><input type="checkbox" name="filter_image_featured_only" value="1" <?php checked(isset($_GET['filter_image_featured_only'])); ?> /><span><?php echo esc_html__('Featured images only', 'optivra-image-studio'); ?></span></label>
			<label class="catalogue-image-studio-filter-check"><input type="checkbox" name="filter_image_gallery_only" value="1" <?php checked(isset($_GET['filter_image_gallery_only'])); ?> /><span><?php echo esc_html__('Gallery images only', 'optivra-image-studio'); ?></span></label>
			<label class="catalogue-image-studio-filter-check"><input type="checkbox" name="filter_include_category" value="1" <?php checked(! empty($filters['include_category'])); ?> /><span><?php echo esc_html__('Include category images', 'optivra-image-studio'); ?></span></label>
			<div class="catalogue-image-studio-filter-actions"><button class="button button-primary"><?php echo esc_html__('Apply filters', 'optivra-image-studio'); ?></button></div>
		</form>
		<?php
	}
	...
	private function render_scan_slot_row(array $slot): void {
		...
		$job           = $this->plugin->jobs()->find_by_slot($slot);
		$status        = $job ? $this->format_status((string) $job['status']) : __('Ready to queue', 'optivra-image-studio');
		...
		<tr>
			...
			<td><?php $this->render_thumbnail((string) ($slot['image_url'] ?? wp_get_attachment_image_url($attachment_id, 'thumbnail')), __('Product image', 'optivra-image-studio')); ?></td>
			<td><strong><?php echo esc_html($product_name); ?></strong><br /><a href="<?php echo esc_url(get_edit_post_link($product_id)); ?>"><?php echo esc_html__('Edit product', 'optivra-image-studio'); ?></a></td>
	...
	private function render_queue_tab(array $usage): void {
		...
		?>
		<div class="catalogue-image-studio-panel">
			<h2><?php echo esc_html__('Queue', 'optivra-image-studio'); ?></h2>
			...
			<?php if (! empty($settings['auto_refresh_job_status']) && ! empty($jobs)) : ?>
				<p class="catalogue-image-studio-help"><?php echo esc_html__('This queue refreshes automatically while jobs are active.', 'optivra-image-studio'); ?></p>
			...
			<form method="post" action="">
				...
				<div class="catalogue-image-studio-toolbar">
					<select name="catalogue_image_studio_action">
						<option value="process"><?php echo esc_html__('Process selected', 'optivra-image-studio'); ?></option>
						<option value="process_next_batch"><?php echo esc_html__('Process next batch', 'optivra-image-studio'); ?></option>
						<option value="retry"><?php echo esc_html__('Retry failed', 'optivra-image-studio'); ?></option>
						<option value="cancel"><?php echo esc_html__('Cancel selected', 'optivra-image-studio'); ?></option>
					</select>
					<button type="submit" class="button button-primary"><?php echo esc_html__('Apply', 'optivra-image-studio'); ?></button>
				</div>
	...
	private function render_review_tab(): void {
		...
		?>
		<div class="catalogue-image-studio-panel">
			<h2><?php echo esc_html__('Optivra Image Studio — Review & Approve', 'optivra-image-studio'); ?></h2>
			<form method="post" action="">
				<?php wp_nonce_field('catalogue_image_studio_action', 'catalogue_image_studio_action_nonce'); ?>
				<div class="catalogue-image-studio-toolbar">
					<select name="catalogue_image_studio_action">
						<option value="approve"><?php echo esc_html__('Approve all selected', 'optivra-image-studio'); ?></option>
						<option value="reject"><?php echo esc_html__('Reject selected', 'optivra-image-studio'); ?></option>
						<option value="retry"><?php echo esc_html__('Retry selected', 'optivra-image-studio'); ?></option>
						<option value="regenerate_seo"><?php echo esc_html__('Regenerate SEO selected', 'optivra-image-studio'); ?></option>
						<option value="revert"><?php echo esc_html__('Revert selected', 'optivra-image-studio'); ?></option>
					</select>
					<button type="submit" class="button button-primary"><?php echo esc_html__('Apply', 'optivra-image-studio'); ?></button>
				</div>
				<?php $this->render_jobs_table($jobs, true); ?>
	...
	private function render_settings_tab(array $settings, array $usage): void {
		...
		<div class="optivra-settings-page">
			<div class="optivra-settings-header">
				<div>
					<h2><?php echo esc_html__('Optivra Image Studio Settings', 'optivra-image-studio'); ?></h2>
					<p><?php echo esc_html__('Configure how Optivra scans, processes, reviews and publishes product images.', 'optivra-image-studio'); ?></p>
				</div>
				<span class="optivra-status-pill <?php echo is_wp_error($usage) ? 'is-disconnected' : 'is-connected'; ?>"><?php echo is_wp_error($usage) ? esc_html__('Not connected', 'optivra-image-studio') : esc_html__('Connected', 'optivra-image-studio'); ?></span>
			</div>
			<?php if (! is_wp_error($usage)) : ?>
				<div class="optivra-summary-grid">
					<?php $this->render_summary_card(__('Plan', 'optivra-image-studio'), (string) ($usage['plan'] ?? __('Unknown', 'optivra-image-studio'))); ?>
					<?php $this->render_summary_card(__('Credits', 'optivra-image-studio'), (string) ((int) ($usage['credits_remaining'] ?? 0) . ' / ' . (int) ($usage['credits_total'] ?? 0))); ?>
					<?php $this->render_summary_card(__('Reset date', 'optivra-image-studio'), (string) ($usage['current_period_end'] ?? __('Not available', 'optivra-image-studio'))); ?>
				</div>
			<?php endif; ?>
			<form method="post" action="" class="optivra-settings-form">
				...
				<section class="optivra-card">
					<div class="optivra-card-header"><h3><?php echo esc_html__('Connection', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Connect this store with the Site API Token from your Optivra account.', 'optivra-image-studio'); ?></p></div>
					<div class="optivra-connection-grid">
						<div>
							<?php $this->render_text_setting('api_token', __('Site API Token', 'optivra-image-studio'), __('Leave this blank to keep the existing saved token.', 'optivra-image-studio'), '', 'password', ! empty($settings['api_token']) ? __('Token saved - leave blank to keep existing token', 'optivra-image-studio') : __('Paste your Site API Token', 'optivra-image-studio')); ?>
							<?php if (! empty($settings['api_token'])) : ?><p class="optivra-muted"><?php echo esc_html(sprintf(__('Saved token: %s', 'optivra-image-studio'), $this->mask_token((string) $settings['api_token']))); ?></p><?php endif; ?>
							<div class="optivra-button-row">
								<button type="submit" name="connect_store" value="1" class="button button-primary"><?php echo esc_html__('Connect', 'optivra-image-studio'); ?></button>
								<button type="submit" name="test_connection" value="1" class="button"><?php echo esc_html__('Test connection', 'optivra-image-studio'); ?></button>
								<?php if (! empty($settings['api_token'])) : ?><button type="submit" name="disconnect_store" value="1" class="button optivra-danger-button"><?php echo esc_html__('Disconnect', 'optivra-image-studio'); ?></button><?php endif; ?>
							</div>
							<div class="optivra-link-row">
								<a href="<?php echo esc_url($this->get_sites_url(is_array($usage) ? $usage : [], $settings)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Get API token', 'optivra-image-studio'); ?></a>
								<a href="<?php echo esc_url(trailingslashit((string) $this->get_app_base_url($usage, $settings)) . 'signup'); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Create an Optivra account', 'optivra-image-studio'); ?></a>
							</div>
						</div>
				...
				<section class="optivra-card">
					<div class="optivra-card-header"><h3><?php echo esc_html__('Processing Defaults', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Choose the default scan, queue and publish behaviour for product images.', 'optivra-image-studio'); ?></p></div>
					<?php $this->render_toggle_setting('require_approval', __('Require review before replacing images', 'optivra-image-studio'), __('Processed images wait for approval before WooCommerce product images are replaced.', 'optivra-image-studio'), ! empty($settings['require_approval'])); ?>
					<?php $this->render_toggle_setting('auto_process_new_images', __('Auto-process newly scanned images', 'optivra-image-studio'), __('Newly discovered images can be queued for processing automatically.', 'optivra-image-studio'), ! empty($settings['auto_process_new_images'])); ?>
					<?php $this->render_toggle_setting('process_featured_images', __('Include featured/product images', 'optivra-image-studio'), __('Scan and queue main product images by default.', 'optivra-image-studio'), ! empty($settings['process_featured_images'])); ?>
					<?php $this->render_toggle_setting('process_gallery_images', __('Include gallery images', 'optivra-image-studio'), __('Scan and queue WooCommerce gallery images by default.', 'optivra-image-studio'), ! empty($settings['process_gallery_images'])); ?>
					<?php $this->render_toggle_setting('process_category_images', __('Include category thumbnail images', 'optivra-image-studio'), __('Allow scans to include product category thumbnails when selected.', 'optivra-image-studio'), ! empty($settings['process_category_images'])); ?>
					<?php $this->render_toggle_setting('duplicate_detection', __('Duplicate detection', 'optivra-image-studio'), __('Reuse previous processed results when the same source image is encountered.', 'optivra-image-studio'), ! empty($settings['duplicate_detection'])); ?>
					<?php $this->render_toggle_setting('pause_on_low_credits', __('Pause processing when credits are low', 'optivra-image-studio'), __('Stop larger queue batches before credits are exhausted.', 'optivra-image-studio'), ! empty($settings['pause_on_low_credits'])); ?>
					<?php $this->render_toggle_setting('retry_failed_jobs', __('Retry failed jobs automatically', 'optivra-image-studio'), __('Keep failed jobs ready for a quick retry pass.', 'optivra-image-studio'), ! empty($settings['retry_failed_jobs'])); ?>
					<?php $this->render_toggle_setting('auto_refresh_job_status', __('Auto-refresh job status', 'optivra-image-studio'), __('Refresh queue status while jobs are active.', 'optivra-image-studio'), ! empty($settings['auto_refresh_job_status'])); ?>
					<?php $this->render_number_setting('batch_size', __('Batch size', 'optivra-image-studio'), __('How many queued images to process at once.', 'optivra-image-studio'), (int) ($settings['batch_size'] ?? 10), 1, 50); ?>
				</section>

				<section class="optivra-card">
					<div class="optivra-card-header"><h3><?php echo esc_html__('Background', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Choose an Optivra preset or upload a brand background from the Media Library.', 'optivra-image-studio'); ?></p></div>
					<?php $this->render_select_setting('background_source', __('Background source', 'optivra-image-studio'), __('Use an Optivra preset or your own uploaded background image.', 'optivra-image-studio'), ['preset' => __('Optivra preset', 'optivra-image-studio'), 'custom' => __('Custom uploaded background', 'optivra-image-studio')], (string) ($settings['background_source'] ?? 'preset')); ?>
					<?php $this->render_select_setting('background_preset', __('Background preset', 'optivra-image-studio'), __('Default background style when using Optivra presets.', 'optivra-image-studio'), $this->get_background_presets(), (string) ($settings['background_preset'] ?? 'optivra-default')); ?>
					<div class="optivra-setting-row">
						<div><strong class="optivra-setting-label"><?php echo esc_html__('Custom background upload', 'optivra-image-studio'); ?></strong><p class="optivra-setting-description"><?php echo esc_html__('Preview stays contained here; the full image is never rendered in the admin screen.', 'optivra-image-studio'); ?></p></div>
						<div class="optivra-setting-control">
							...
							<div class="optivra-button-row"><button type="button" class="button" id="catalogue-image-studio-pick-background"><?php echo esc_html__('Choose background', 'optivra-image-studio'); ?></button><button type="button" class="button" id="catalogue-image-studio-remove-background"><?php echo esc_html__('Remove background', 'optivra-image-studio'); ?></button></div>
				...
				<section class="optivra-card">
					<div class="optivra-card-header"><h3><?php echo esc_html__('Framing', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Global framing defaults. Image-to-edge is controlled per queued image.', 'optivra-image-studio'); ?></p></div>
					<?php $this->render_select_setting('default_scale_mode', __('Default image framing', 'optivra-image-studio'), __('Choose the default amount of space around the product.', 'optivra-image-studio'), $this->get_scale_modes(), (string) ($settings['default_scale_mode'] ?? 'auto')); ?>
					<?php $this->render_toggle_setting('smart_scaling', __('Smart product framing', 'optivra-image-studio'), __('Let Optivra keep products comfortably framed without clipping.', 'optivra-image-studio'), ! empty($settings['smart_scaling'])); ?>
					<?php $this->render_number_setting('framing_padding', __('Product padding / safe margin', 'optivra-image-studio'), __('Default safe margin around products, expressed as a percentage.', 'optivra-image-studio'), (int) ($settings['framing_padding'] ?? 8), 0, 30); ?>
					<?php $this->render_toggle_setting('preserve_transparent_edges', __('Preserve transparent product edges', 'optivra-image-studio'), __('Avoid trimming subtle transparent edges unless a job override says otherwise.', 'optivra-image-studio'), ! empty($settings['preserve_transparent_edges'])); ?>
				</section>

				<section class="optivra-card">
					<div class="optivra-card-header"><h3><?php echo esc_html__('Shadow', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Control the product shadow style used during processing.', 'optivra-image-studio'); ?></p></div>
					<?php $this->render_select_setting('shadow_mode', __('Shadow mode', 'optivra-image-studio'), __('Off disables shadow entirely. Under product creates a contact shadow; behind product adds depth.', 'optivra-image-studio'), $this->get_shadow_modes(), (string) ($settings['shadow_mode'] ?? 'under')); ?>
					<?php $this->render_select_setting('shadow_strength', __('Shadow strength', 'optivra-image-studio'), __('Preset shadow intensity.', 'optivra-image-studio'), $this->get_shadow_strengths(), (string) ($settings['shadow_strength'] ?? 'medium')); ?>
					<details class="optivra-subdetails"><summary><?php echo esc_html__('Advanced shadow settings', 'optivra-image-studio'); ?></summary>
						<?php $this->render_range_setting('shadow_opacity', __('Shadow opacity', 'optivra-image-studio'), __('Opacity percentage for custom shadows.', 'optivra-image-studio'), (int) ($settings['shadow_opacity'] ?? 23), 0, 100); ?>
						<?php $this->render_range_setting('shadow_blur', __('Shadow blur', 'optivra-image-studio'), __('Blur radius for soft edges.', 'optivra-image-studio'), (int) ($settings['shadow_blur'] ?? 22), 0, 80); ?>
						<?php $this->render_number_setting('shadow_offset_x', __('Shadow offset X', 'optivra-image-studio'), __('Horizontal shadow offset in pixels.', 'optivra-image-studio'), (int) ($settings['shadow_offset_x'] ?? 0), -300, 300); ?>
						<?php $this->render_number_setting('shadow_offset_y', __('Shadow offset Y', 'optivra-image-studio'), __('Vertical shadow offset in pixels.', 'optivra-image-studio'), (int) ($settings['shadow_offset_y'] ?? 0), -300, 300); ?>
						<?php $this->render_range_setting('shadow_spread', __('Shadow spread / scale', 'optivra-image-studio'), __('Relative spread percentage for the shadow.', 'optivra-image-studio'), (int) ($settings['shadow_spread'] ?? 100), 25, 200); ?>
						<?php $this->render_range_setting('shadow_softness', __('Shadow softness', 'optivra-image-studio'), __('Extra softness applied to shadow edges.', 'optivra-image-studio'), (int) ($settings['shadow_softness'] ?? 60), 0, 100); ?>
						<?php $this->render_text_setting('shadow_color', __('Shadow colour', 'optivra-image-studio'), __('Default is black.', 'optivra-image-studio'), (string) ($settings['shadow_color'] ?? '#000000'), 'color'); ?>
					</details>
				</section>

				<section class="optivra-card">
					<div class="optivra-card-header"><h3><?php echo esc_html__('Lighting Enhancement', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Improve product lighting before the final image is saved.', 'optivra-image-studio'); ?></p></div>
					<?php $this->render_toggle_setting('lighting_enabled', __('Enable lighting enhancement', 'optivra-image-studio'), __('Apply lighting correction during processing.', 'optivra-image-studio'), ! empty($settings['lighting_enabled'])); ?>
					<?php $this->render_select_setting('lighting_mode', __('Enhancement mode', 'optivra-image-studio'), __('Choose whether lighting corrections target the product or full image.', 'optivra-image-studio'), $this->get_lighting_modes(), (string) ($settings['lighting_mode'] ?? 'auto')); ?>
					<?php $this->render_range_setting('brightness_correction', __('Brightness correction', 'optivra-image-studio'), __('Adjust brightness from -100 to 100.', 'optivra-image-studio'), (int) ($settings['brightness_correction'] ?? 0), -100, 100); ?>
					<?php $this->render_range_setting('contrast_correction', __('Contrast correction', 'optivra-image-studio'), __('Adjust contrast from -100 to 100.', 'optivra-image-studio'), (int) ($settings['contrast_correction'] ?? 0), -100, 100); ?>
					<?php $this->render_toggle_setting('highlight_recovery', __('Highlight recovery', 'optivra-image-studio'), __('Reduce harsh highlights where possible.', 'optivra-image-studio'), ! empty($settings['highlight_recovery'])); ?>
					<?php $this->render_toggle_setting('shadow_lift', __('Shadow lift', 'optivra-image-studio'), __('Recover detail in dark product areas.', 'optivra-image-studio'), ! empty($settings['shadow_lift'])); ?>
					<?php $this->render_toggle_setting('neutralize_tint', __('Colour balance / neutralise tint', 'optivra-image-studio'), __('Balance colour casts for more natural product colour.', 'optivra-image-studio'), ! empty($settings['neutralize_tint'])); ?>
					<?php $this->render_select_setting('lighting_strength', __('Strength', 'optivra-image-studio'), __('Overall lighting enhancement strength.', 'optivra-image-studio'), $this->get_lighting_strengths(), (string) ($settings['lighting_strength'] ?? 'medium')); ?>
				</section>

				<section class="optivra-card">
					<div class="optivra-card-header"><h3><?php echo esc_html__('SEO Metadata', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Control generated filenames and attachment metadata.', 'optivra-image-studio'); ?></p></div>
					<?php $this->render_toggle_setting('generate_seo_filename', __('Generate SEO filename', 'optivra-image-studio'), __('Rename processed files to cleaner, search-friendly filenames.', 'optivra-image-studio'), ! empty($settings['generate_seo_filename'])); ?>
					<?php $this->render_toggle_setting('generate_alt_text', __('Generate alt text', 'optivra-image-studio'), __('Fill image alt text suggestions automatically.', 'optivra-image-studio'), ! empty($settings['generate_alt_text'])); ?>
					<?php $this->render_toggle_setting('generate_image_title', __('Generate image title', 'optivra-image-studio'), __('Fill the WordPress media title with the suggested image title.', 'optivra-image-studio'), ! empty($settings['generate_image_title'])); ?>
					<?php $this->render_toggle_setting('generate_caption', __('Generate caption', 'optivra-image-studio'), __('Generate an attachment caption when available.', 'optivra-image-studio'), ! empty($settings['generate_caption'])); ?>
					<?php $this->render_toggle_setting('generate_description', __('Generate description', 'optivra-image-studio'), __('Generate an attachment description when available.', 'optivra-image-studio'), ! empty($settings['generate_description'])); ?>
					<?php $this->render_toggle_setting('only_fill_missing_metadata', __('Only fill missing metadata', 'optivra-image-studio'), __('Leave existing attachment metadata alone unless a field is blank.', 'optivra-image-studio'), ! empty($settings['only_fill_missing_metadata'])); ?>
					<?php $this->render_toggle_setting('overwrite_existing_metadata', __('Override existing metadata', 'optivra-image-studio'), __('Allow generated metadata to replace existing values. This cannot be combined with only-fill-missing.', 'optivra-image-studio'), ! empty($settings['overwrite_existing_metadata'])); ?>
					<?php $this->render_text_setting('brand_keyword_suffix', __('Brand keyword suffix', 'optivra-image-studio'), __('Optional brand or collection term to append to generated SEO suggestions.', 'optivra-image-studio'), (string) ($settings['brand_keyword_suffix'] ?? '')); ?>
					<?php $this->render_seo_preview_example($settings); ?>
				</section>

				<section class="optivra-card">
					<div class="optivra-card-header"><h3><?php echo esc_html__('Notifications', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Choose how Optivra reports completed and failed work.', 'optivra-image-studio'); ?></p></div>
					<?php $this->render_toggle_setting('show_completion_alerts', __('Show admin notices for completed batches', 'optivra-image-studio'), __('Display success notices after processing or approval actions complete.', 'optivra-image-studio'), ! empty($settings['show_completion_alerts'])); ?>
					<?php $this->render_toggle_setting('show_failed_alerts', __('Show admin notices for failed jobs', 'optivra-image-studio'), __('Display failure notices when the API or processing workflow returns an error.', 'optivra-image-studio'), ! empty($settings['show_failed_alerts'])); ?>
					<?php $this->render_toggle_setting('email_batch_complete', __('Email admin when batch completes', 'optivra-image-studio'), __('Send an email when a processing batch finishes.', 'optivra-image-studio'), ! empty($settings['email_batch_complete'])); ?>
					<?php $this->render_toggle_setting('email_job_failed', __('Email admin when job fails', 'optivra-image-studio'), __('Send an email when an image job fails.', 'optivra-image-studio'), ! empty($settings['email_job_failed'])); ?>
					<?php $this->render_text_setting('notification_email', __('Notification email address', 'optivra-image-studio'), __('Optional. Defaults to the WordPress admin email if left blank.', 'optivra-image-studio'), (string) ($settings['notification_email'] ?? ''), 'email'); ?>
				</section>

				<?php $this->render_advanced_settings($settings); ?>
				<div class="optivra-save-bar"><button type="submit" class="button button-primary"><?php echo esc_html__('Save Settings', 'optivra-image-studio'); ?></button></div>
			</form>
		</div>
		<?php
	}
	...
	private function render_logs_tab(): void {
		...
		?>
		<div class="catalogue-image-studio-panel">
			<h2><?php echo esc_html__('Optivra Image Studio Diagnostics', 'optivra-image-studio'); ?></h2>
			<div class="catalogue-image-studio-log-grid">
				<section><h3><?php echo esc_html__('Connection logs', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Connection attempts are recorded in WooCommerce logs under source catalogue-image-studio. Secrets are never written to logs.', 'optivra-image-studio'); ?></p></section>
				<section><h3><?php echo esc_html__('Job logs', 'optivra-image-studio'); ?></h3><p><?php echo esc_html__('Recent failed job messages appear below. Full job events are available in WooCommerce logs.', 'optivra-image-studio'); ?></p></section>
			</div>
			<button type="button" class="button" onclick="navigator.clipboard && navigator.clipboard.writeText(document.getElementById('catalogue-image-studio-diagnostics').textContent);"><?php echo esc_html__('Copy support diagnostics', 'optivra-image-studio'); ?></button>
	...
	private function render_jobs_table(array $jobs, bool $selectable): void {
		$empty_message = $selectable
			? __('Run your first scan to find WooCommerce product images.', 'optivra-image-studio')
			: __('No failed jobs to show.', 'optivra-image-studio');
		?>
		<table class="widefat fixed striped catalogue-image-studio-jobs">
			<thead>
				<tr>
					...
					<th><?php echo esc_html__('Product', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('Before', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('After', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('Slot', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('Status', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('SEO filename / alt preview', 'optivra-image-studio'); ?></th><th><?php echo esc_html__('Progress / message', 'optivra-image-studio'); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php if (empty($jobs)) : ?>
	...
	private function render_tabs(string $active_tab): void {
		$tabs = [
			'dashboard' => __('Dashboard', 'optivra-image-studio'),
			'scan'      => __('Scan Catalogue', 'optivra-image-studio'),
			'queue'     => __('Queue', 'optivra-image-studio'),
			'review'    => __('Review & Approve', 'optivra-image-studio'),
			'settings'  => __('Settings', 'optivra-image-studio'),
			'logs'      => __('Logs', 'optivra-image-studio'),
		];
		?>
	...
	private function get_usage() {
		...
		if (empty($settings['api_base_url']) || empty($settings['api_token'])) {
			return new WP_Error('catalogue_image_studio_not_connected', __('Not connected.', 'optivra-image-studio'));
		}
	...
	private function render_settings_panel(array $settings, $usage): void {
		...
		<div class="catalogue-image-studio-panel catalogue-image-studio-onboarding">
			...
			<h2><?php echo esc_html__('Connect your WooCommerce store to Optivra Image Studio', 'optivra-image-studio'); ?></h2>
			<p><?php echo esc_html__('Paste your Site API Token from your Optivra account to connect this store.', 'optivra-image-studio'); ?></p>
	...
	private function render_external_service_disclosure(): void {
		...
		<div class="catalogue-image-studio-disclosure">
			<strong><?php echo esc_html__('External service disclosure', 'optivra-image-studio'); ?></strong>
			<p><?php echo esc_html__('Optivra Image Studio connects your WooCommerce store to Optivra\'s external image processing service. When you scan or process images, selected product image data, image URLs, product names, categories, and related metadata may be sent to Optivra for image background replacement, optimisation, review workflow support, and SEO metadata generation. An Optivra account and Site API Token are required.', 'optivra-image-studio'); ?></p>
			<p><?php echo esc_html__('No product or image data is sent until you connect your Site API Token and intentionally start scanning or processing. Image processing uses credits from your Optivra account.', 'optivra-image-studio'); ?></p>
			<div class="catalogue-image-studio-link-row">
				<a href="<?php echo esc_url('https://www.optivra.app'); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Service: Optivra Image Studio', 'optivra-image-studio'); ?></a>
				<a href="<?php echo esc_url(CIS_TERMS_URL); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Terms of Service', 'optivra-image-studio'); ?></a>
				<a href="<?php echo esc_url(CIS_PRIVACY_URL); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Privacy Policy', 'optivra-image-studio'); ?></a>
				<a href="<?php echo esc_url(CIS_DATA_URL); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Data Processing', 'optivra-image-studio'); ?></a>
				<a href="<?php echo esc_url(CIS_SUPPORT_URL); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Support', 'optivra-image-studio'); ?></a>
	...
	private function render_advanced_settings(array $settings): void {
		...
		<details class="catalogue-image-studio-advanced optivra-card">
			<summary><?php echo esc_html__('Advanced (for support / development only)', 'optivra-image-studio'); ?></summary>
			<p class="catalogue-image-studio-warning"><?php echo esc_html__('These settings are for development/support use only. Most users should not change them.', 'optivra-image-studio'); ?></p>
			<?php $this->render_text_setting('api_base_url_override', __('API Base URL override', 'optivra-image-studio'), __('Only change this when support asks you to point the plugin at a different Optivra API.', 'optivra-image-studio'), (string) ($settings['api_base_url_override'] ?? ''), 'url'); ?>
			<?php $this->render_toggle_setting('debug_mode', __('Debug mode', 'optivra-image-studio'), __('Enable detailed logging for support and troubleshooting.', 'optivra-image-studio'), ! empty($settings['debug_mode'])); ?>
			<div class="optivra-button-row">
				<button type="submit" name="clear_local_cache" value="1" class="button"><?php echo esc_html__('Clear local cache', 'optivra-image-studio'); ?></button>
				<button type="submit" name="reset_local_data" value="1" class="button optivra-danger-button"><?php echo esc_html__('Reset plugin local data', 'optivra-image-studio'); ?></button>
			</div>
		</details>
		<?php
	}
	...
	private function render_connection_status($usage, bool $connected): void {
		if (! $connected || is_wp_error($usage)) {
			...
			<div class="catalogue-image-studio-status-card catalogue-image-studio-status-card-disconnected">
				<strong><?php echo esc_html__('Not connected', 'optivra-image-studio'); ?></strong>
				<p><?php echo esc_html__('Connect your Optivra account to view credits and process images.', 'optivra-image-studio'); ?></p>
			</div>
			<?php
			return;
		}
	...
	private function render_usage($usage): void {
		...
		<div class="catalogue-image-studio-status-card catalogue-image-studio-status-card-connected">
			<strong><?php echo esc_html__('Connected', 'optivra-image-studio'); ?></strong>
			<?php if (! empty($usage['domain']) && is_string($usage['domain'])) : ?>
				<p><?php echo esc_html(sprintf(__('Connected domain: %s', 'optivra-image-studio'), $usage['domain'])); ?></p>
			<?php endif; ?>
		</div>
		<div class="catalogue-image-studio-usage">
			<div>
				<span><?php echo esc_html__('Plan', 'optivra-image-studio'); ?></span>
				<strong><?php echo esc_html(ucfirst((string) ($usage['plan'] ?? 'unknown'))); ?></strong>
			</div>
			<div>
				<span><?php echo esc_html__('Credits', 'optivra-image-studio'); ?></span>
				<strong><?php echo esc_html((string) ($usage['credits_remaining'] ?? 0)); ?> / <?php echo esc_html((string) ($usage['credits_total'] ?? 0)); ?></strong>
			</div>
			<div>
				<span><?php echo esc_html__('Status', 'optivra-image-studio'); ?></span>
				<strong><?php echo esc_html(ucfirst((string) ($usage['subscription_status'] ?? 'unknown'))); ?></strong>
			</div>
			<div>
				<span><?php echo esc_html__('Reset date', 'optivra-image-studio'); ?></span>
				<strong>
					<?php
					...
					echo esc_html($period_ts ? date_i18n(get_option('date_format'), $period_ts) : __('Unavailable', 'optivra-image-studio'));
					?>
				</strong>
			</div>
		</div>
		...
		?>
		<div class="catalogue-image-studio-credit-meter" aria-label="<?php echo esc_attr__('Credits remaining', 'optivra-image-studio'); ?>">
			...
			echo esc_html(
				$next_reset
					? sprintf(
						...
						__('Next reset: %s', 'optivra-image-studio'),
						date_i18n(get_option('date_format'), $next_reset)
					)
					: __('Next reset date unavailable.', 'optivra-image-studio')
			);
			?>
		</p>
		<div class="catalogue-image-studio-cta-buttons">
			<a class="button button-primary" href="<?php echo esc_url($this->get_upgrade_url($usage)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Upgrade plan', 'optivra-image-studio'); ?></a>
			<a class="button" href="<?php echo esc_url($this->get_upgrade_url($usage)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Manage billing', 'optivra-image-studio'); ?></a>
		</div>
		<?php
	}
	...
	private function render_workflow_panel(array $jobs, $usage, array $settings, int $processable_count): void {
		...
		<div class="catalogue-image-studio-panel catalogue-image-studio-panel-wide">
			<div class="catalogue-image-studio-workflow-steps">
				<div class="catalogue-image-studio-workflow-step <?php echo $connected ? 'catalogue-image-studio-workflow-step-complete' : ''; ?>"><span>1</span><?php echo esc_html__('Connect', 'optivra-image-studio'); ?></div>
				<div class="catalogue-image-studio-workflow-step"><span>2</span><?php echo esc_html__('Scan', 'optivra-image-studio'); ?></div>
				<div class="catalogue-image-studio-workflow-step"><span>3</span><?php echo esc_html__('Optimise', 'optivra-image-studio'); ?></div>
				<div class="catalogue-image-studio-workflow-step"><span>4</span><?php echo esc_html__('Review & approve', 'optivra-image-studio'); ?></div>
			</div>
			<div class="catalogue-image-studio-toolbar">
				<h2><?php echo esc_html__('Images', 'optivra-image-studio'); ?></h2>
				<form method="post" action="">
					<?php wp_nonce_field('catalogue_image_studio_action', 'catalogue_image_studio_action_nonce'); ?>
					<input type="hidden" name="catalogue_image_studio_action" value="scan" />
					<button type="submit" class="button" <?php disabled(! $connected); ?>><?php echo esc_html__('Scan Product Images', 'optivra-image-studio'); ?></button>
				</form>
			</div>

			<?php if (! $connected) : ?>
				<div class="catalogue-image-studio-empty-state">
					<?php echo esc_html__('Connect your Optivra account, then scan your product catalogue to find images ready for optimisation.', 'optivra-image-studio'); ?>
				</div>
			<?php else : ?>
				...
				<div class="tablenav top">
					<div class="alignleft actions">
						<select name="catalogue_image_studio_action" <?php disabled(! $connected); ?>>
							<option value="process"><?php echo esc_html__('Process selected', 'optivra-image-studio'); ?></option>
							<option value="approve"><?php echo esc_html__('Approve selected', 'optivra-image-studio'); ?></option>
							<option value="reject"><?php echo esc_html__('Reject selected', 'optivra-image-studio'); ?></option>
							<option value="revert"><?php echo esc_html__('Revert selected', 'optivra-image-studio'); ?></option>
						</select>
						<button type="submit" class="button button-primary" <?php disabled(! $connected); ?>><?php echo esc_html__('Apply', 'optivra-image-studio'); ?></button>
					</div>
				</div>
					...
					<table class="widefat fixed striped catalogue-image-studio-jobs">
						<thead>
							<tr>
								...
								<th><?php echo esc_html__('Product', 'optivra-image-studio'); ?></th>
								<th><?php echo esc_html__('Before', 'optivra-image-studio'); ?></th>
								<th><?php echo esc_html__('After', 'optivra-image-studio'); ?></th>
								<th><?php echo esc_html__('Slot', 'optivra-image-studio'); ?></th>
								<th><?php echo esc_html__('Status', 'optivra-image-studio'); ?></th>
								<th><?php echo esc_html__('SEO', 'optivra-image-studio'); ?></th>
								<th><?php echo esc_html__('Updated', 'optivra-image-studio'); ?></th>
							</tr>
						</thead>
						<tbody>
							<?php if (empty($jobs)) : ?>
								<tr><td colspan="8"><?php echo esc_html__('Run your first scan to find WooCommerce product images.', 'optivra-image-studio'); ?></td></tr>
							<?php else : ?>
								<?php foreach ($jobs as $job) : ?>
	...
	private function render_monetisation_prompt($usage, int $credits_remaining, int $processable_count, bool $insufficient): void {
		...
		<div class="catalogue-image-studio-monetisation <?php echo $insufficient ? 'catalogue-image-studio-monetisation-warning' : ''; ?>">
			<div>
				<strong>
					...
					echo esc_html(
						sprintf(
							...
							__('You have %d images left to optimise.', 'optivra-image-studio'),
							$credits_remaining
						)
					);
					?>
				...
				<p>
					<?php echo esc_html__('Selected:', 'optivra-image-studio'); ?>
					<span data-cis-selected-count>0</span>
					<?php echo esc_html__('image(s). Available credits:', 'optivra-image-studio'); ?>
					<?php echo esc_html((string) $credits_remaining); ?>.
				</p>
				<?php if ($insufficient) : ?>
					<p><?php echo esc_html(sprintf(
						/* translators: %d: processable images */
						__('Process %d now, then buy credits or upgrade plan. Upgrade to process your full catalogue.', 'optivra-image-studio'),
						$available_now
					)); ?></p>
				<?php else : ?>
					<p><?php echo esc_html__('Upgrade to process your full catalogue.', 'optivra-image-studio'); ?></p>
				<?php endif; ?>
			</div>
			<div class="catalogue-image-studio-cta-buttons">
				<a class="button button-primary" href="<?php echo esc_url($this->get_upgrade_url($usage)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Upgrade plan', 'optivra-image-studio'); ?></a>
				<a class="button" href="<?php echo esc_url($this->get_buy_credits_url($usage)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Buy credits', 'optivra-image-studio'); ?></a>
				<a class="button" href="<?php echo esc_url($this->get_account_url($usage)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Manage account', 'optivra-image-studio'); ?></a>
			</div>
		</div>
		<?php
	}
		...
		<div class="catalogue-image-studio-connection-card">
			<?php if ($wrap_form) : ?><form method="post" action="" class="catalogue-image-studio-connection-form"><?php else : ?><div class="catalogue-image-studio-connection-form"><?php endif; ?>
				...
				<label class="catalogue-image-studio-token-field" for="catalogue-image-studio-api-token">
					<span><?php echo esc_html__('Site API Token', 'optivra-image-studio'); ?></span>
					<input
						type="password"
						...
						class="regular-text"
						value=""
						placeholder="<?php echo esc_attr(! empty($settings['api_token']) ? __('Token saved - leave blank to keep it', 'optivra-image-studio') : __('Paste your Site API Token', 'optivra-image-studio')); ?>"
						autocomplete="new-password"
					/>
					<?php if (! empty($settings['api_token'])) : ?>
						<small class="catalogue-image-studio-help"><?php echo esc_html(sprintf(__('Saved token: %s', 'optivra-image-studio'), $this->mask_token((string) $settings['api_token']))); ?></small>
					<?php endif; ?>
				</label>

				<div class="catalogue-image-studio-link-row">
					<a href="<?php echo esc_url($this->get_sites_url(is_array($usage) ? $usage : [], $settings)); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Get your API token', 'optivra-image-studio'); ?></a>
					<a href="<?php echo esc_url(trailingslashit((string) $this->get_app_base_url($usage, $settings)) . 'signup'); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__('Create an Optivra account', 'optivra-image-studio'); ?></a>
				</div>

				<div class="catalogue-image-studio-actions">
					<button type="submit" name="connect_store" value="1" class="button button-primary"><?php echo esc_html__('Connect', 'optivra-image-studio'); ?></button>
					<button type="submit" name="test_connection" value="1" class="button"><?php echo esc_html__('Test', 'optivra-image-studio'); ?></button>
					<?php if (! empty($settings['api_token'])) : ?>
						<button type="submit" name="disconnect_store" value="1" class="button catalogue-image-studio-danger-link"><?php echo esc_html__('Disconnect', 'optivra-image-studio'); ?></button>
					<?php endif; ?>
				</div>
	...
	private function render_category_presets_summary(array $settings): void {
		...
		if (empty($presets)) {
			echo '<p class="catalogue-image-studio-help">' . esc_html__('No category presets saved yet. Categories without a preset use your defaults.', 'optivra-image-studio') . '</p>';
			return;
		}
		...
		foreach ($presets as $category_id => $preset) {
			...
			echo '<span>' . esc_html($this->get_scale_modes()[(string) ($preset['scale_mode'] ?? 'auto')] ?? __('Auto', 'optivra-image-studio')) . '</span>';
			echo '<span>' . esc_html($this->get_background_presets()[(string) ($preset['background_preset'] ?? 'optivra-default')] ?? __('Optivra Default', 'optivra-image-studio')) . '</span>';
			echo '</div>';
		}
		echo '</div>';
	}
	...
	private function render_seo_preview_example(array $settings): void {
		...
		<div class="catalogue-image-studio-example">
			<strong><?php echo esc_html__('Preview example', 'optivra-image-studio'); ?></strong>
			<p><?php echo esc_html($example_filename . '.webp'); ?></p>
			<p><?php echo esc_html__('Alt text: Black Leather Wallet product image', 'optivra-image-studio'); ?></p>
		</div>
		<?php
	}
	...
	private function get_background_presets(): array {
		return [
			'optivra-default' => __('Optivra Default', 'optivra-image-studio'),
			'soft-white'      => __('Soft White', 'optivra-image-studio'),
			'cool-studio'     => __('Cool Studio', 'optivra-image-studio'),
			'warm-studio'     => __('Warm Studio', 'optivra-image-studio'),
		];
	}

	private function get_scale_modes(): array {
		return [
			'auto'     => __('Auto', 'optivra-image-studio'),
			'tight'    => __('Tight', 'optivra-image-studio'),
			'balanced' => __('Balanced', 'optivra-image-studio'),
			'loose'    => __('Loose', 'optivra-image-studio'),
			'close-up' => __('Close-up', 'optivra-image-studio'),
			'wide'     => __('Wide', 'optivra-image-studio'),
			'tall'     => __('Tall', 'optivra-image-studio'),
		];
	}

	private function get_shadow_modes(): array {
		return [
			'off'    => __('Off', 'optivra-image-studio'),
			'under'  => __('Under product', 'optivra-image-studio'),
			'behind' => __('Behind product', 'optivra-image-studio'),
			'custom' => __('Custom', 'optivra-image-studio'),
		];
	}

	private function get_shadow_strengths(): array {
		return [
			'light'  => __('Light', 'optivra-image-studio'),
			'medium' => __('Medium', 'optivra-image-studio'),
			'strong' => __('Strong', 'optivra-image-studio'),
			'custom' => __('Custom', 'optivra-image-studio'),
		];
	}
	...
	private function get_lighting_modes(): array {
		return [
			'auto'          => __('Auto', 'optivra-image-studio'),
			'product-only'  => __('Product only', 'optivra-image-studio'),
			'whole-image'   => __('Whole image', 'optivra-image-studio'),
		];
	}

	private function get_lighting_strengths(): array {
		return [
			'low'    => __('Low', 'optivra-image-studio'),
			'medium' => __('Medium', 'optivra-image-studio'),
			'high'   => __('High', 'optivra-image-studio'),
			'custom' => __('Custom', 'optivra-image-studio'),
		];
	}
	...
	private function render_job_row(array $job, bool $selectable = true): void {
		...
		$product_title    = $product_id ? get_the_title($product_id) : __('Unknown product', 'optivra-image-studio');
		?>
		<tr>
			<?php if ($selectable) : ?>
			...
			<td>
				...
				<a href="<?php echo esc_url(get_edit_post_link($product_id)); ?>"><?php echo esc_html__('Edit product', 'optivra-image-studio'); ?></a>
			</td>
			<td><?php $this->render_thumbnail($before_url, __('Before', 'optivra-image-studio')); ?></td>
	...
	private function render_job_edge_controls(array $job): void {
		...
		<details class="optivra-job-options">
			<summary><?php echo esc_html__('Framing overrides', 'optivra-image-studio'); ?></summary>
			<label class="optivra-inline-check"><input type="checkbox" name="edge_overrides[<?php echo esc_attr((string) $job_id); ?>][enabled]" value="1" <?php checked(! empty($job['edge_to_edge_enabled'])); ?> /> <?php echo esc_html__('Image to edge', 'optivra-image-studio'); ?></label>
			<small class="catalogue-image-studio-help"><?php echo esc_html__('Choosing any edge automatically enables the override for this image.', 'optivra-image-studio'); ?></small>
			<div class="optivra-edge-grid">
				<label><input type="checkbox" name="edge_overrides[<?php echo esc_attr((string) $job_id); ?>][left]" value="1" <?php checked(! empty($job['edge_to_edge_left'])); ?> /> <?php echo esc_html__('Left edge', 'optivra-image-studio'); ?></label>
				<label><input type="checkbox" name="edge_overrides[<?php echo esc_attr((string) $job_id); ?>][right]" value="1" <?php checked(! empty($job['edge_to_edge_right'])); ?> /> <?php echo esc_html__('Right edge', 'optivra-image-studio'); ?></label>
				<label><input type="checkbox" name="edge_overrides[<?php echo esc_attr((string) $job_id); ?>][top]" value="1" <?php checked(! empty($job['edge_to_edge_top'])); ?> /> <?php echo esc_html__('Top edge', 'optivra-image-studio'); ?></label>
				<label><input type="checkbox" name="edge_overrides[<?php echo esc_attr((string) $job_id); ?>][bottom]" value="1" <?php checked(! empty($job['edge_to_edge_bottom'])); ?> /> <?php echo esc_html__('Bottom edge', 'optivra-image-studio'); ?></label>
			</div>
			<?php if ('completed' === (string) ($job['status'] ?? '')) : ?>
				<small class="catalogue-image-studio-help"><?php echo esc_html__('Process this image again to apply changed framing overrides.', 'optivra-image-studio'); ?></small>
			<?php endif; ?>
		</details>
		<?php
	}
	...
	private function render_seo_fields(array $job): void {
		$job_id = (int) $job['id'];
		$fields = [
			'filename'    => __('Filename', 'optivra-image-studio'),
			'alt_text'    => __('Alt', 'optivra-image-studio'),
			'title'       => __('Title', 'optivra-image-studio'),
			'caption'     => __('Caption', 'optivra-image-studio'),
			'description' => __('Description', 'optivra-image-studio'),
		];
		$values = [
	...
	private function render_thumbnail(string $url, string $label): void {
		if ('' === $url) {
			echo '<span class="catalogue-image-studio-muted">' . esc_html__('None', 'optivra-image-studio') . '</span>';
			return;
		}
	...
	private function render_after_thumbnail(array $links, array $job): void {
		...
		if (in_array($status, ['unprocessed', 'queued', 'processing'], true)) {
			...
			echo '<span>' . esc_html__('Not processed yet', 'optivra-image-studio') . '</span>';
			echo '<small>' . esc_html__('Process this queued image to create an after preview.', 'optivra-image-studio') . '</small>';
			echo '</div>';
			return;
		}
		...
		if ('' === $links['preview'] || '' === $links['full']) {
			...
			echo '<span>' . esc_html__('Processed image file missing', 'optivra-image-studio') . '</span>';
			echo '<small>' . esc_html__('Reprocess this image before approving.', 'optivra-image-studio') . '</small>';
			echo '<button type="submit" class="button button-small" name="catalogue_image_studio_action" value="retry" onclick="var box=this.closest(\'tr\').querySelector(\'.catalogue-image-studio-job-check\'); if (box) { box.checked = true; }">' . esc_html__('Reprocess', 'optivra-image-studio') . '</button>';
			echo '</div>';
			return;
		}
		...
		<a href="<?php echo esc_url($links['full']); ?>" target="_blank" rel="noopener noreferrer" class="catalogue-image-studio-after-link">
			<img src="<?php echo esc_url($links['preview']); ?>" alt="<?php echo esc_attr__('After', 'optivra-image-studio'); ?>" class="catalogue-image-studio-thumb" />
		</a>
		<?php if ('remote' === $links['source']) : ?>
			<small class="catalogue-image-studio-help"><?php echo esc_html__('Remote preview. Approving will import this file into Media Library.', 'optivra-image-studio'); ?></small>
		<?php endif; ?>
		<?php
	}
	...
	private function render_job_diagnostics(array $job): void {
		$rows = [
			__('Job ID', 'optivra-image-studio')                  => (string) ($job['id'] ?? ''),
			__('Product ID', 'optivra-image-studio')              => (string) ($job['product_id'] ?? ''),
			__('Original attachment ID', 'optivra-image-studio')  => (string) ($job['original_attachment_id'] ?? $job['attachment_id'] ?? ''),
			__('Processed attachment ID', 'optivra-image-studio') => (string) ($job['processed_attachment_id'] ?? ''),
			__('Processed URL', 'optivra-image-studio')           => (string) ($job['processed_url'] ?? ''),
			__('Storage bucket', 'optivra-image-studio')          => (string) ($job['processed_storage_bucket'] ?? ''),
			__('Storage path', 'optivra-image-studio')            => (string) ($job['processed_storage_path'] ?? ''),
			__('Last processing error', 'optivra-image-studio')   => (string) ($job['error_message'] ?? ''),
			__('Last approval error', 'optivra-image-studio')     => (string) ($job['approval_error'] ?? ''),
		];
		?>
		<details class="optivra-job-diagnostics">
			<summary><?php echo esc_html__('Technical details', 'optivra-image-studio'); ?></summary>
			<dl>
				<?php foreach ($rows as $label => $value) : ?>
					<dt><?php echo esc_html($label); ?></dt>
					<dd><?php echo '' !== trim($value) ? esc_html($value) : esc_html__('Not stored', 'optivra-image-studio'); ?></dd>
				<?php endforeach; ?>
			</dl>
		</details>
		<?php
